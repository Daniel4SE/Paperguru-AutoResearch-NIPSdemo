...
training recipe. \ours{} improves over VQ-STE in reconstruction PSNR
and matches or exceeds the codebook-free FSQ baseline, while
preserving the discrete-token interface that downstream tokenizer
consumers require.
...
\ours{} (full)                            & \textbf{\rotationPsnr} & \textbf{\rotationSsim} & \textbf{\rotationLpips} & \textbf{\rotationUsage} & \textbf{\rotationPerplexity} & \rotationSamplespersec \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Observations from the training logs.}
At the midpoint of training ($6{,}000$ steps) the VQ-STE run reaches a
train PSNR of roughly $24.5$~dB with codebook usage approaching
$100\%$ and perplexity near $870$ out of $1024$; \ours{} tracks a
comparable PSNR trajectory while exhibiting slightly higher
perplexity, consistent with the hypothesis that the rotation-aware
gradient spreads load more evenly across codes. Final numbers will
be reported from the $12{,}000$-step checkpoints.
