...
$\rmat$ is never materialised. A component-wise ablation on
CIFAR-10 yields an unexpected diagnosis: naively back-propagating
the full $s\cdot\rmat$ Jacobian induces a sharp codebook collapse
after $\sim\!1{,}000$ steps, which we trace to a positive-feedback
interaction between the rotation and the commitment loss. The
ablation localises the failure precisely to the Householder matrix
$\rmat$ in the backward pass; the scalar rescale $s$ is harmless.
We then show that a soft-assignment entropy regulariser is
sufficient to restore $100\%$ codebook utilisation under the full
rotation-aware Jacobian, recovering $+1.4$\,dB of validation PSNR
over the collapsed baseline. We present controlled comparisons on
CIFAR-10 against VQ-VAE with STE, FSQ, and Gumbel-Softmax VQ, with
all four methods trained under an identical recipe on the same
H100 GPU. Reference implementation, configurations, and training
logs for every reported run are released publicly.
\end{abstract}
